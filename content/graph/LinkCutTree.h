...
struct SplayTree {
    struct Node {
        int ch[2] = {0, 0}, p = 0, self = 0, path = 0, sub = 0, vir = 0;
        bool flip = 0;
    };
    vector<Node> T;
...
struct LinkCut : SplayTree {
    ...
    int subTree(int u, int v) {
        ++u, ++v; reroot(v); access(u); return T[u].vir + T[u].self;
    }
    // puts v at root and path to u is in l e f t ch i ld
    // returns path aggregate
    int path(int u, int v) {
        ++u, ++v; reroot(u); access(v); return T[v].path;
    }
    void update(int u, int v) {
        ++u; access(u); T[u].self = v; pull(u);
    }
};